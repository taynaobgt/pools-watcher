#!/usr/bin/env python3
"""
pools_watcher_bot_gha.py  (ban cho GitHub Actions - co bao cao dinh ky)
--------------------------------------------------------------------------
Ban CHAY MOT LAN (single-run), thiet ke de GitHub Actions goi theo lich (cron).

Moi lan workflow chay:
  1. Tai trang, tinh hash + kiem tra "coming soon"
  2. So sanh voi state.json (duoc luu & commit lai vao repo giua cac lan chay)
  3. Neu co thay doi trang thai / noi dung -> gui tin Telegram ngay
  4. [MOI] Neu da du STATUS_REPORT_INTERVAL_MINUTES phut ke tu lan bao cao
     truoc -> gui them 1 tin "bao cao dinh ky" du co thay doi hay khong
  5. Ghi de state.json -> workflow se commit file nay lai

Vi GitHub Actions khong giu tien trinh chay lien tuc, viec "moi 60 phut
gui bao cao" duoc mo phong bang cach luu thoi diem bao cao gan nhat vao
state.json (committed trong repo) va so sanh voi thoi gian hien tai moi
lan job duoc kich hoat. Vi vay workflow can duoc lap lich chay THUONG
XUYEN HON chu ky bao cao (vi du moi 5-10 phut) de dam bao khong bi le
qua muc thoi han 60 phut.
"""
...
URL = os.environ.get("WATCH_URL", "https://pools.trade/")
BOT_TOKEN = os.environ.get("TELEGRAM_BOT_TOKEN", "8810625890:AAGI9SnQl_H1-X1cQec6KHCA6TvSfcaX8yw")
CHAT_ID = os.environ.get("TELEGRAM_CHAT_ID", "2013455551")
STATE_FILE = os.environ.get("STATE_FILE", "state.json")
REQUEST_TIMEOUT = 15
COMING_SOON_MARKERS = ["coming soon", "something new"]
...
def maybe_send_periodic_report(state: dict, current_coming_soon, status_code, changed_this_round: bool) -> None:
    ...
    status_text = "Van dang 'Coming soon'" if current_coming_soon else "KHONG con 'Coming soon' (co the da mo!)"
    ...
    send_telegram_message(
        "<b>Bao cao dinh ky - Pools Watcher (GitHub Actions)</b>\n\n"
        f"URL: {URL}\n"
